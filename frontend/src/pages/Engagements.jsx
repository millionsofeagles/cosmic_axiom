...
import dummyEngagements from "../test_data/engagementsTestData";

function Engagements() {
    const [engagements, setEngagements] = useState([]);

    useEffect(() => {
        setEngagements(dummyEngagements);
    }, []);
    ...
    return (
        <DashboardLayout>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
                <div className="bg-indigo-100 dark:bg-indigo-800 text-indigo-700 dark:text-indigo-300 rounded-lg p-4 shadow-sm text-center transition hover:shadow-md">
                    <div className="text-2xl font-bold">{statusCounts["Active"] || 0}</div>
                    <div className="text-sm">Active</div>
                </div>
                <div className="bg-green-100 dark:bg-green-800 text-green-700 dark:text-green-300 rounded-lg p-4 shadow-sm text-center transition hover:shadow-md">
                    ...
                    <tbody>
                        {engagements.map((engagement) => (
                            <tr key={engagement.id} className="hover:bg-gray-100 dark:hover:bg-gray-700 transition">
                                ...
                                <td className="p-4 text-sm text-gray-700 dark:text-gray-400">{engagement.client}</td>
                                <td className="p-4 text-sm">
                                    <span className={`px-2 py-1 rounded text-xs font-medium ${engagement.status === "Active"
