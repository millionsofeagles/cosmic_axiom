import { Edit3, Loader2, Calendar, Building2, Shield, Clock, CheckCircle, AlertCircle, XCircle, FileText } from "lucide-react";
...
function RoEList() {
    ...
    const [loadingRoeId, setLoadingRoeId] = useState(null);
    ...
    // Helper function to get RoE status styling
    const getRoeStatusStyles = (status) => {
        switch (status) {
            case 'APPROVED':
                return {
                    icon: CheckCircle,
    ...
    useEffect(() => {
        const fetchRoEs = async () => {
            try {
                ...
                if (!res.ok) throw new Error("Failed to load RoE documents");

                const data = await res.json();
                setRoes(data);
            } catch (err) {
                console.error("Error fetching RoE documents:", err);
            }
        };
    ...
    const handleViewRoE = async (roeId, engagementId) => {
        setLoadingRoeId(roeId);
        try {
            navigate(`/engagements/${engagementId}/roe/${roeId}/preview`);
        } catch (err) {
            console.error("Error navigating to RoE:", err);
            setLoadingRoeId(null);
        }
    };
        ...
        <DashboardLayout>
            {loadingRoeId && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
                    <div className="flex flex-col items-center bg-white dark:bg-gray-800 px-6 py-4 rounded-lg shadow-md">
                        ...
                        <p className="text-sm text-gray-800 dark:text-gray-200">Loading RoE document...</p>
                    </div>
                </div>
            )}
            ...
            <div className="mb-6">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
                    <div>
                        <h2 className="text-3xl font-bold text-gray-900 dark:text-gray-100">Rules of Engagement</h2>
                        <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
                            Manage and track all your Rules of Engagement documents
                        </p>
                    </div>
                    <Link
                        to="/engagements"
                ...
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-gray-500 dark:text-gray-400">
                        {filteredRoEs.length} of {roes.length} documents
                    </span>
                </div>
            </div>
            ...
            {filteredRoEs.length === 0 ? (
                <div className="text-center py-12">
                    ...
                    <h3 className="mt-2 text-sm font-medium text-gray-900 dark:text-gray-100">No RoE documents found</h3>
                    <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                        {searchQuery ? "Try adjusting your search criteria" : "Get started by creating a new RoE document"}
                    </p>
                    {!searchQuery && (
                        <div className="mt-6">
                            <Link
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    const engagementStatus = roe.engagement?.status;
                    const statusStyle = getStatusStyles(engagementStatus);
                    const StatusIcon = statusStyle.icon;
                    
                    const roeStatusStyle = getRoeStatusStyles(roe.status);
                    const RoeStatusIcon = roeStatusStyle.icon;
                        ...
                        >
                            ...
                            <div className="p-6">
                                    ...
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div className="flex-1 space-y-1">
                                            <div className="flex items-center justify-between">
                                                <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-medium border ${roeStatusStyle.className}`}>
                                                    <RoeStatusIcon className="w-3 h-3" />
                                                    {roe.status === 'DRAFT' ? 'Draft' :
                                                     roe.status === 'APPROVED' ? 'Approved' :
                                                     roe.status === 'ACTIVE' ? 'Active' :
                                                     roe.status === 'EXPIRED' ? 'Expired' :
                                                     roe.status === 'ARCHIVED' ? 'Archived' :
                                                     roe.status}
                                                </span>
                                        ...
                                        <div className="flex-1 space-y-1">
                                            <div className="flex items-center justify-between text-xs">
                                                ...
                                                <span className="text-gray-700 dark:text-gray-300">
                                                    {roe.createdAt ? new Date(roe.createdAt).toLocaleDateString() : "N/A"}
                                                </span>
                                            </div>
                                            <div className="flex items-center justify-between text-xs">
                                                <span className="text-gray-500 dark:text-gray-400">Updated</span>
                                                <span className="text-gray-700 dark:text-gray-300">
                                                    {roe.updatedAt ? new Date(roe.updatedAt).toLocaleDateString() : "N/A"}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div className="flex items-center justify-between pt-4 border-t border-gray-100 dark:border-gray-700">
                                    <button
                                        onClick={() => handleViewRoE(roe.id, roe.engagementId)}
                                    ...
                                    {roe.engagement?.id && (
                                        <Link
                                            to={`/engagements/${roe.engagementId}/roe/${roe.id}`}
                                            title="Edit RoE"
